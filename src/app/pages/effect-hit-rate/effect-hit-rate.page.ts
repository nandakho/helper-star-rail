...
export class EffectHitRatePage implements OnInit {
  ...
  setTag(){
    ...
    if(this.typeSelected==0){
      desc += `What's your effect hit chance?${this.url?`\nBase Chance: ${this.myStats.baseChance}%, Enemy's Resist: ${this.myStats.enemiesResist}%, Resist Reduction: ${this.myStats.resistReduc}%, EHR: ${this.myStats.ehr}%\nEffect Hit Chance: ${this.result.realChance}%`:''}`;
    } else {
      desc += `How many EHR is needed?${this.url?`\nBase Chance: ${this.myStats.baseChance}%, Enemy's Resist: ${this.myStats.enemiesResist}%, Resist Reduction: ${this.myStats.resistReduc}%\nEHR Needed: ${this.result.ehrNeeded}%`:''}`;
    }
